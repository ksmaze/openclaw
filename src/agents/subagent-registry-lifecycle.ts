...
export function createSubagentRegistryLifecycleController(params: {
  ...
  const finalizeSubagentCleanup = async (
    ...
    const now = Date.now();
    const freshDescendants = Math.max(0, params.countPendingDescendantRuns(entry.childSessionKey));
    ...
    const activeDescendants = bailedForDescendants
      ? Math.max(freshDescendants, 1)
      : freshDescendants;
